...
def main(argv):
    ...
    results_directory = "parsed-results"
    os.makedirs(results_directory, exist_ok=True)

    for result_dir in os.listdir(argv[1]):
        result_path = os.path.join(argv[1], result_dir)

        files_pool = Pool(cpu_count() * 2)
        files_pool.map(func=partial(parse_file, result_path),
                       iterable=filter(lambda x: '.xml' in x, os.listdir(os.path.abspath(result_path)))
                       )
        ...
        results_file = os.path.join(results_directory, result_dir, f'flows.json')
