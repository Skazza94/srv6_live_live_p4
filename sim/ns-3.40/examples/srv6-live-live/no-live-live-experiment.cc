...
main(int argc, char* argv[])
{
    ...
    // congestionControl));
    Config::SetDefault("ns3::TcpSocket::SndBufSize", UintegerValue(2 << 16));
    Config::SetDefault("ns3::TcpSocket::RcvBufSize", UintegerValue(2 << 16));
    ...
    if (activeFlows > 0)
    {
        for (uint32_t i = 0; i < 2; i++)
        {
            ApplicationContainer activeReceiverApp =
                createSinkTcpApplication(activePort, activeReceivers.Get(i));
            activeReceiverApp.Start(Seconds(0.0));
            activeReceiverApp.Stop(Seconds(flowEndTime + 1));

            ApplicationContainer activeSenderApp =
                createTcpApplication(activeReceiverIpv6Interfaces[i]->GetAddress(2).GetAddress(),
                                     activePort,
                                     activeSenders.Get(i),
                                     activeRateTcp,
                                     maxBytes,
            ...
            Simulator::Schedule(Seconds(1.1),
                                ...
                                getPath(rtxPath, "active-1-rtx.data"));
        }

        if (!alternate)
        {
            for (uint32_t i = 2; i < activeFlows; i++)
            {
                ApplicationContainer activeReceiverApp =
        ...
        else
        {
            for (uint32_t i = 1; i < backupFlows; i++)
            {
                ...
                backupSenderApp = createUdpApplication(
                    ...
                    backupSenders.Get(i),
                    backupRateUdp,
                    !isMoreThanHalf ? 3.0 : 5.0,
                    !isMoreThanHalf ? 3.99 : 5.99,
