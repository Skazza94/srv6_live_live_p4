...
def run_test(test_folder: str, delay: int, loss: float, test_number: int):
    ...
    assigned_client_ips = []
    for idx, ip in enumerate(available_client_ips):
        if idx == 10:
            break
    ...
    assigned_server_ips = []
    for idx, ip in enumerate(available_server_ips):
        if idx == 10:
            break
    ...
    for ip in assigned_server_ips:
        logging.info(f"/bin/bash -c '/usr/bin/iperf3 -6 -s -B {ip}'")
        kathara.exec(
            machine_name="server",
    ...
    for idx, ip in enumerate(assigned_client_ips):
        logging.info(f"/bin/bash -c 'iperf3 -6 -c {assigned_server_ips[idx]} -b 6M -J -B {ip}'")
        iperf_clients_stats.append(kathara.exec(
            machine_name="client",
            command=shlex.split(f"/bin/bash -c 'iperf3 -6 -c {assigned_server_ips[idx]} -b 6M -J -B {ip}'"),
            lab_hash=lab.hash
        ))
    ...
    iperf_clients_stats = list(map(lambda x: get_output(x), iperf_clients_stats))
