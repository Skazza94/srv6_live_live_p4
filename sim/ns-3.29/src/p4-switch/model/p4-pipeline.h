...
/**
   ...
   * This is coming from ns3-bmv2, but I added support for egress processing too.
   */
...
private:
  ...
  /**
     * \brief Invoke the P4 processing ingress pipeline (parser, match-action, deparser)
     */
  void process_ingress (std::unique_ptr<bm::Packet> packet, uint32_t ingress_port);

  /**
     * \brief Invoke the P4 processing egress pipeline (parser, match-action, deparser)
     */
  ...
  /**
     ...
     */
  std::unique_ptr<bm::Packet> get_bm_packet (Ptr<const Packet> ns3_packet);
